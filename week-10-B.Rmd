...
mod_1_wfl %>% 
  fit(week_10_train) %>% 
  predict(week_10_train) %>% 
  bind_cols(week_10_train %>% 
              select(reg_chg)) %>% 
...
Our model was overfitting on week_10_train since we were fitting the model to the same data we were using to estimate the RMSE. Especially if you use a super complex polynomial, since the points are different, the distance between the points will be drastically different with a new model, thus drastically increasing the error of your model. 
...
```{r}
new_obs <- tibble(treatment = "local", poverty_n = 1.5)
...
We are 95% confident that the true registration change would fall in between these two numbers (Bayesian sense). Another way to interpret it (Frequentist sense) would be that if we ran a ton of different experiments, 95% of them would spit out a prediction within this interval. 
